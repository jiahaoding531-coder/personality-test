    ...
    <>
      ...
      */}
      <div className="card">
        ...
        {/*
          ⚠️ 这段话必须跟着功能走，每加一个功能就回来看一眼。
          它已经错两次了：先是写着"不接真实地图和天气"（接了高德之后就成了
          当面撒谎——用户会在结果页看到真实地名和天气，却先被告知"没有这些"），
          然后写着"AI 生成解读是下一步"（而它已经做完了）。

          自己说出去的话和产品实际做的事对不上，比没有这句话更糟：
          用户会先怀疑自己看错了，再怀疑这东西是不是坏的。
        */}
        <p className="note">
          景点用的是 59 个杭州模拟数据；定位、天气和导航接了高德。
          排序由算法算出（分数、因子都能展开查账），AI 只负责把理由说成人话
          ——它读的就是那份算好的依据，不参与打分。
        </p>
      </div>
    </>
